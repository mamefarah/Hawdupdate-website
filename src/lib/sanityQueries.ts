...
export function sanityHealthCheck(): Promise<string | null> {
  return sanityClient.fetch(groq`*[_type == "homepage"][0]._id`)
}
...
export function fetchMission(): Promise<MissionDocument | null> {
  return sanityClient.fetch(groq`*[_type == "mission"][0]{_id, title, description}`)
}

export function fetchVision(): Promise<VisionDocument | null> {
  return sanityClient.fetch(groq`*[_type == "vision"][0]{_id, title, description}`)
}
